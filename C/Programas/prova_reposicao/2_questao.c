...
/*
2) (2 pontos) Refaça o programa anterior solicitando ao usuário a quantidade de
itens a serem armazenados (valor entre 10 e 1000, repetindo a solicitação caso
o usuário digite um valor errado) e aloque dinamicamente o  array que 
armazenará os elementos. Altere o restante do código para que funcione com o 
novo array e salve em um novo arquivo.
*/
...
void verificador_resposta_3(int *resposta){ // criando uma função de nome "verificacao_resposta" que recebe um ponteiro de variável do tipo char com nome "resposta"

    if (resposta < 10 || resposta > 1000){ 
        ...
        while (true){ // while funcionará até a condição do "if" ser verdadeira
            ...
            printf(cor_branca "Quantos contatos deseja salvar? \n"
                              "Valor entre 10 e 1000.\n\n");
            printf(cor_vermelha ">> " cor_branca);
            scanf("%d", &resposta);
            fflush(stdin);
...
void cadastrar_contato(struct informacao_agenda *agenda, int *quantidade_contatos, int resposta_quantidade){
    
    if (*quantidade_contatos < resposta_quantidade - 1){
...
int main() {
    ...
    printf(cor_amarela ">> " cor_branca);
    fflush(stdin);
    scanf("%d", &resposta_quantidade);
    fflush(stdin);
    verificador_resposta_3(resposta_quantidade);
    printf(cor_branca "\n================================================================\n\n" cor_branca);
    Sleep(2000);

    agenda = malloc (resposta_quantidade * sizeof(struct informacao_agenda));
    ...
    while (resposta[0] != '4'){
        ...
        if (resposta[0] == '1'){ // Cadastrar novo item
            cadastrar_contato(agenda, &quantidade_contatos, resposta_quantidade);
        }
