...
int main(void){
    ...
    if ((texto[0] != 'C') && (texto[0] != 'c')){ // verificação se a entrada do usuário é válida
        printf(cor_vermelha "\nESCOLHA INVALIDA!\n" cor_branca); // mostrando "ESCOLHA INVALIDA" em vermelho
        Sleep(1000); // sleep de 1 segundo
        nome2(); 
        while (repeticao_l == 1){ // caso a condição seja verdadeira o while irá repetir seu conteúdo 
            printf("Tipo:"
            "\nOpcao 1"
            "\nOpcao 2\n"); // menu do "Tipo"
            printf("\nDigite a opcao: ");  // mostrando "Digite o numero da conversao"
            scanf("%s", &texto[0]); // entrada de dado pelo usuário
            if ((texto[0] == 'P') || (texto[0] == 'p') || 
                (texto[0] == 'C') || (texto[0] == 'c')){ // condição para sair do while
                    repeticao_l = 0; // repeticao_l receberá o valor 0
