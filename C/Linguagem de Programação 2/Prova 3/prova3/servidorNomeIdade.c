...
/*
    2 : Faça um servidor que:
        - Exista garantia de entrega entre as informações e o estabelecimento de conexão.
        - Receba um nome (char) e a idade (int) e retorne essas duas informações.
        - Quando o servidor receber "exit" ou 0 (zero), ele deve ser finalizado.
*/

typedef struct identidade
{
    int idade;
    char nome[TAMANHO_MENSAGEM];
} Identidades;

Identidades pessoas[QUANTIDADE_CLIENTES];
...
void *funcaoCliente(void *valor)
{
    ...
    while (true)
    {
        ...
        bzero(&pessoas[meuClienteFuncao], sizeof(pessoas[numero]));

        // & Recebendo o nome
        resultado = recv(meuClienteFuncao, pessoas[numero].nome, TAMANHO_MENSAGEM, 0);

        if (resultado > 0)
        {
            if (strncmp("exit", pessoas[numero].nome, 4) == 0)
            {
                contadorClientes -= 1;
            ...
            wprintf(corAzul L"[C]" corBranca " : Nome  : " corAzul "%s \n", pessoas[numero].nome);
        }

        // & Recebendo a idade
        resultado = recv(meuClienteFuncao, buffer, TAMANHO_MENSAGEM, 0);
        pessoas[numero].idade = atoi(buffer);

        if (resultado > 0)
        {
            if (pessoas[numero].idade == 0)
            {
                contadorClientes -= 1;
                close(meuClienteFuncao);
                wprintf(corAmarela L"Programa finalizado!" corBranca);
            ...
            wprintf(corAzul L"[C]" corBranca " : Idade : " corAzul "%d \n", pessoas[numero].idade);
        }

        char mensagemServidor[TAMANHO_MENSAGEM];
        sprintf(mensagemServidor, "%s : %d \n", pessoas[numero].nome, pessoas[numero].idade);
        ...
        wprintf(L"\n[Pessoa {%d}: " corVerde "%s" corBranca ", " corVerde "%d" corBranca "]", numero, pessoas[numero].nome, pessoas[numero].idade);
        traco(2);
...
int main()
{
    ...
    {
        bzero(&pessoas[i], sizeof(pessoas[i]));
